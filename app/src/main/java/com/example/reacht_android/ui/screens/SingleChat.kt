...
fun SingleChat(navController: NavController, viewModel: AppViewModel) {
    ...
    Column(
        ...
        // Lista de mensajes
        LazyColumn(
            state = listState,  // El estado del scroll
            modifier = Modifier
                .weight(1f)
            ...
            Button(
                onClick = {
                    if (inputText.isNotBlank()) {
                        // Se envía a servidor -> registra en BD y servidor envía en broadcast a todos, nosotros incluidos
                        // Lo recibiremos por hilo de escucha de SocketClient que meterá los NEW_CHAT_MESSAGE en cola de mensajes
                        // Cola que leeremos en hilo lector de dentro de enterChat() del VM
